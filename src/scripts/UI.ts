...
export class UI {
  ...
  private init() {
    ...
    this.text.setTintFill(0xffd300).setOrigin(0.5, -0.5).setDepth(33)

    this.button = this.scene.add.sprite(width / 2, Number(this.scene.game.config.height / 2), 'restart').setOrigin(0).setInteractive().setVisible(false).setDepth(30)
    this.button.on('pointerdown', this.onClick, this)
    this.button.x -= this.button.width / 2
    this.uiRect = this.scene.add.graphics().setDepth(29).setVisible(false);
    this.uiRect.fillStyle(0x042A3D, 0.9);
  ...
  public showRestartButton() {
    ...
    this.uiRect.setVisible(true).setAlpha(0.9)
  }
}
